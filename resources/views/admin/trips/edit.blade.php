    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
          <form action="{{route("trips.update", $trip->id)}}" method="post" class="mt-6 space-y-6">
            @csrf
            <div>
            ...
            <p class="text-center text-sm">
              Actualice la información del viaje llenando el siguiente formulario
            </p>
              ...
              <div class="w-1/2">
                ...
                <select name="origin" id="origin" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                  ...
                  @foreach ($terminals as $terminal)
                    <option 
                      value="{{$terminal->id}}" 
                      {{$terminal->id === $trip->origin ? "selected" : ""}}
                    >
                      {{$terminal->name}}
                    </option>
                  @endforeach
              ...
              <div class="w-1/2">
                ...
                <select name="destination" id="destination" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                  ...
                  @foreach ($terminals as $terminal)
                    <option 
                      value="{{$terminal->id}}"
                      {{$terminal->id === $trip->destination ? "selected" : ""}}
                    >
                      {{$terminal->name}}
                    </option>
                  @endforeach
            ...
            <div class="flex gap-10 py-3 w-full">
              <div class="w-1/2">
                ...
                <x-text-input id="output_date" name="output_date" type="date" class="mt-1 block w-full" :value="$trip->output_date" required autofocus />
              </div>
              <div class="w-1/2">
                <x-input-error class="my-2" :messages="$erros->get("output_time")" /> 
                <x-input-label for="output_time" :value="__('Hora de Salida')" />
                <x-text-input id="output_time" name="output_time" type="time" class="mt-1 block w-full" :value="$trip->output_time" required autofocus />
              </div>
            </div>
            <div class="w-full flex gap-10"> 
              <div class="w-1/2">
                ...
                <x-text-input id="arrival_date" name="arrival_date" type="date" class="mt-1 block w-full" :value="$trip->arrival_date" required autofocus />
              </div>
              <div class="w-1/2">
                <x-input-error class="my-2" :messages="$erros->get("arrival_time")" /> 
                <x-input-label for="arrival_time" :value="__('Hora de Llegada')" />
                <x-text-input id="arrival_time" name="arrival_time" type="time" class="mt-1 block w-full" :value="$trip->arrival_time" required autofocus />
              </div>
            </div>
            <div class="w-full flex gap-10">
              <div class="w-1/2">
