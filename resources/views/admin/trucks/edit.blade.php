    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 text-gray-900">
          <form action="{{route("trucks.update", $truck->id)}}" method="post" class="mt-6 space-y-6">
            @csrf
            <div>
              <legend class="text-center font-bold font-outtfit text-3xl">
                Editar Camión: {{$truck->brand . " - " . $truck->model}}
              </legend>
            </div>
            <p class="text-center text-sm">
              Actualice la información del camión llenando el siguiente formulario
            </p>
            <div class="flex gap-10 w-full items-center">
              <div class="w-1/2">
                  ...
                  <x-text-input id="brand" name="brand" type="text" class="mt-1 block w-full" :value="$truck->brand" required autofocus />
              </div>
              <div class="w-1/2">
                  <x-input-error class="my-2" :messages="$erros->get("model")" />
                  <x-input-label for="model" :value="__('Modelo')" />
                  <x-text-input id="model" name="model" type="text" class="mt-1 block w-full" :value="$truck->model" required autofocus />
              </div>
            </div>
            <div class="w-full flex gap-10"> 
              <div class="w-1/2">
                  ...
                  <x-text-input id="plate" name="plate" type="text" class="mt-1 block w-full" :value="$truck->plate" required autofocus />
              </div>
              <div class="w-1/2">
                  <x-input-error class="my-2" :messages="$erros->get("capacity")" /> 
                  <x-input-label for="capacity" :value="__('Número de acientos')" />
                  <x-text-input id="capacity" name="capacity" type="number" class="mt-1 block w-full" :value="$truck->capacity" required autofocus />
                </div>
              </div>
            </div>
            <div class="flex gap-10 px-5 py-3 w-full">
              ...
              <div class="w-1/2">
                ...
                <x-input-label for="status" :value="__('Estado del Cambión')" />
                <select name="status" id="status" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                  <option value="0" disabled selected> -- Seleccione -- </option>
                  <option value="1" {{$driver->status === 1 ? "selected" : "" }}>Activo</option>
                  <option value="2" {{$driver->status === 2 ? "selected" : "" }}>Inactivo</option>
                </select>
              </div>
            </div>
