    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg h-full">
        <div class="p-6 text-gray-900">
          <form action="{{route("terminals.store")}}" method="post" class="mt-6 space-y-6">
            @csrf
            <div>
            ...
            <p class="text-center text-sm">
              Registre una nueva Terminal llenando el siguiente formulario
            </p>
            ...
            <div class="w-full flex gap-10"> 
              <div class="w-1/2">
                  ...
                  <x-text-input id="address" name="address" type="text" class="mt-1 block w-full" :value="old('address', '')" required autofocus />
              </div>
              <div class="w-1/2">
                  <x-input-error class="my-2" :messages="$erros->get("cp")" /> 
                  <x-input-label for="cp" :value="__('Código Postal (5 Digitos)')" />
                  <x-text-input id="cp" name="cp" type="number" class="mt-1 block w-full" :value="old('cp', '')" required autofocus />
                </div>
              </div>
            </div>
            <div class="flex gap-10 w-full items-center px-5">
              <div class="w-1/2">
                  ...
                  <x-text-input id="state" name="state" type="text" class="mt-1 block w-full" :value="old('state', '')" required autofocus />
              </div>
              <div class="w-1/2">
            ...
            <div class="flex gap-10 w-full items-center px-5 my-5">
              <div class="w-1/2">
                  ...
                  <x-text-input id="lat" name="lat" type="text" class="mt-1 block w-full" :value="old('lat', '')" required autofocus />
              </div>
              <div class="w-1/2">
                  <x-input-error class="my-2" :messages="$erros->get("lng")" />
                  <x-input-label for="lng" :value="__('Longitud')" />
                  <x-text-input id="lng" name="lng" type="tel" class="mt-1 block w-full" :value="old('lng', '')" required autofocus />
              </div>
            </div>
